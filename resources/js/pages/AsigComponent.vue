...
<td> <a  class="btn btn-secondary " @click="VerStudiantesCurso(curso)"  data-target="#example" data-toggle="modal">
                  ...
                  <td  v-show="curso.id== g.courseid" v-for="g in grupo" :key="g.id">
                    <a v-show="curso.id== g.courseid" class="btn btn-info btn-xs" @click="VerStudiantes(g)"  data-target="#exampleModalCenter" data-toggle="modal">
                   Estudiantes {{g.name}}
                    </a>
                  </td>
  ...
  <table v-if="student.length > 0" class="table">
            ...
            <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
              <div class="modal-content ">
                <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalLongTitle"> {{namec}} </h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
...
export default {
  ...
  methods: {
    ...
    VerStudiantes(g){
        this.idgrup = g.id;
        this.name = g.name;
        axios.post("Administracion/student/"+this.idgrup+"/edit").then((res) => {
